...
class AssetsService
{
  public function getDueAssetsQuery($checklistId = null)
  {
    ...
    $dueCondition = sprintf(
      ...
      DueScheduleQuery::intervalWeek,
      DueScheduleQuery::intervalMonth,
      DueScheduleQuery::intervalHour,
      DueScheduleQuery::dailySchedule
    );
    ...
    $assetsQuery = Asset::query()
      ->select([
        ...
        DB::raw("
            SUM(
                CASE WHEN ({$dueCondition}) THEN 1 ELSE 0 END
            ) AS due_items
        "),
        DB::raw("
            SUM(
                CASE WHEN cir.checked_at IS NOT NULL
                AND NOT ({$dueCondition})
                THEN 1 ELSE 0 END
            ) AS done_items
        "),
      ])
